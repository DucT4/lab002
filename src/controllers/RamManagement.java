...
public class RamManagement implements IItem<Ram> {
    ...
    public List<String> searchBus(String bus) {
        ...
        for (Ram ram : ramList) {
            if (ram.getBus().toLowerCase().contains(bus.toLowerCase())) {
                result.add(String.format(" %-10s%-10s%-20s%-20d", ram.getCode(), ram.getBus(),
                        ram.getProductionMonthYear(), ram.getQuantity()));
            }
        }
        return result;
    }
    ...
    public List<String> searchBrand(String brand) {
        ...
        for (Ram ram : ramList) {
            if (ram.getBrand().toLowerCase().contains(brand.toLowerCase())) {
                result.add(String.format("%-10s%-10s%-20s%-20d", ram.getCode(), ram.getBrand(),
                        ram.getProductionMonthYear(), ram.getQuantity()));
    ...
    public Ram update(String oldId, Ram newItem) {
        ...
        if (!newType.isEmpty() && (newType.equals("LPDDR5") || newType.equals("DDR5") || newType.equals("LPDDR4")
                || newType.equals("DDR4"))) {
        ...
        String newProductionMonthYear = sc.nextLine();
    ...
    public String genCodeType(String type) {
        ...
        // Tạo mã code mới
        // Tạo tiền tố từ 3 ký tự đầu của loại RAM (hoặc ít hơn nếu loại RAM ngắn hơn 3
        // ký tự)
        String prefix = type.toUpperCase();

        // Tạo mã code bằng cách kết hợp tiền tố và số thứ tự (count + 1) được định dạng
        // với 3 chữ số
