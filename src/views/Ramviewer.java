...
public class Ramviewer {
        ...
        while (running) {
            ...
            switch (choice) {
                ...
                case 7:
                    ...
                    break;
                case 8:
                System.out.println("Sort by Bus:");
                    ramManagement.sortRamsByBus();
                    displayAllRams();
                    break;
                case 9:
                    running = false;
    ...
    private void searchRam() {
        ...
        switch (searchChoice) {
            case 1:
                System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
                System.out.println(" Code       Type    Production month year     Quantity");
    ...
    private void displayAllRams() {
        System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
        System.out.println(
                "Code      Type      Bus      Brand      Quantity        Production Month and Year    |       ");
